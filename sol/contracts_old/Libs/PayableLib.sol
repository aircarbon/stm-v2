...
library PayableLib {
	...
	function setIssuerValues(
		...
		require(
			msg.sender == issueBatch.originator || msg.sender == owner,
		...
		require(
			qty_saleAllocation <= current.qty_issuanceMax,
			"Bad cashflow request: qty_saleAllocation too large"
		);
		require(
			qty_saleAllocation >= current.qty_issuanceSold,
			"Bad cashflow request: qty_saleAllocation too small"
		);

		// price is either in eth or in usd
		require(
			(cents_currentPrice == 0 && wei_currentPrice > 0) ||
				(cents_currentPrice > 0 && wei_currentPrice == 0),
			"Bad cashflow request: price either in USD or ETH"
		);
		...
		if (cashflowData.args.cashflowType == StructLib.CashflowType.BOND) {
			if (
				...
				((cashflowData.wei_currentPrice != wei_currentPrice &&
					cashflowData.wei_currentPrice != 0) || cashflowData.cents_currentPrice > 0)
			) {
	...
	function pay(
		...
		require(
			cashflowData.wei_currentPrice > 0 || cashflowData.cents_currentPrice > 0,
			"Bad cashflow request: no price set"
		);
		require(
			cashflowData.wei_currentPrice == 0 || cashflowData.cents_currentPrice == 0,
			"Bad cashflow request: ambiguous price set"
		);
    ...
    */
	...
	function issuerPay(
		...
		// validate count
		require(
			ipbd.curNdx + count <= ld._ledgerOwners.length,
			"Count must be < remaining token holders in the payment batch"
		);

		// disallow extra payments
		require(
			ipbd.curPaymentProcessedAmount <= ipbd.curPaymentTotalAmount,
			"Extra payment(s) have been processed"
		);
		...
		if (ipbd.curNdx == 0) {
			require(
				...
				"New payment initialization error: Reset Payment Total Amount"
			);
		...
		if (cashflowData.args.cashflowType == StructLib.CashflowType.BOND) {
			...
			for (
				ipv.addrNdx = ipv.initAddrNdx;
				ipv.addrNdx < ipv.initAddrNdx + count;
				ipv.addrNdx++
			) {
				...
				if (addr != issueBatch.originator) {
					// exclude issuer from payments
					uint256[] storage stIds = ld._ledger[addr].tokenType_stIds[1];

					for (ipv.stNdx = 0; ipv.stNdx < stIds.length; ipv.stNdx++) {
						ipv.sharePercentage =
							(ipv.amountSubscribed * 10**36) /
							uint256(uint64(ld._sts[stIds[ipv.stNdx]].currentQty));
			...
			emit IssuerPaymentBatchProcessed(
				ipbd.curPaymentId,
				ipbd.curBatchNdx,
				msg.sender,
				msg.value,
			...
			if (ipbd.curPaymentProcessedAmount == ipbd.curPaymentTotalAmount) {
				emit IssuerPaymentProcessed(
					ipbd.curPaymentId,
					msg.sender,
					ipbd.curPaymentTotalAmount,
					ipbd.curBatchNdx
				);
	...
	function getCashflowData(
		...
		if (ld.contractType == StructLib.ContractType.CASHFLOW_BASE) {
			if (ld._batches_currentMax_id == 1) {
				StructLib.SecTokenBatch storage issueBatch = ld._batches[1]; // CFT: uni-batch
				uint256[] storage issuer_stIds = ld._ledger[issueBatch.originator].tokenType_stIds[
					1
				]; // CFT: uni-type
				StructLib.PackedSt storage issuerSt = ld._sts[issuer_stIds[0]];
	...
	function processSubscriberPayment(
		...
		if (cashflowData.wei_currentPrice > 0) {
			...
			require(ethSat_UsdCents != -1 || bnbSat_UsdCents != -1, "Bad usd/{eth|bnb} rate");
			if (ethSat_UsdCents != -1) {
				// use uth/usd rate (ETH Ropsten, mainnet)
				v.weiPrice =
					(cashflowData.cents_currentPrice * 10**24) /
					(uint256(ethSat_UsdCents));
			} else if (bnbSat_UsdCents != -1) {
				// use bnb/usd rate (BSC Mainnet 56 & Testnet 97)
				v.weiPrice =
					(cashflowData.cents_currentPrice * 10**24) /
					(uint256(bnbSat_UsdCents));
			}
		}
		...
		require(
			cashflowData.qty_saleAllocation >=
				cashflowData.qty_issuanceSold + //v.qtyIssuanceSold
					v.qtyTokens,
			"Bad cashflow request: insufficient quantity for sale"
		);
