...
library FuturesLib {
	...
	event SetFeePerContractOverride(
		uint256 tokTypeId,
		address indexed ledgerOwner,
	...
	function setLedgerOverride(
		...
		if (overrideType == 1) {
			...
			feePerContractOverride(ld, std, tokTypeId, ledgerOwner, value);
	}
	...
	function openFtPos(
		...
		require(
			a.qty_A <= type(int64).max &&
				a.qty_B <= type(int64).max &&
				a.qty_A >= type(int64).min &&
				a.qty_B >= type(int64).min &&
				a.qty_A != 0 &&
				a.qty_B != 0,
			"Bad quantity"
		); // min/max signed int64, non-zero
		require(a.qty_A + a.qty_B == 0, "Quantity mismatch");
		require(a.tokTypeId >= 0 && a.tokTypeId <= std._tt_Count, "Bad tokTypeId");
		require(
			std._tt_settle[a.tokTypeId] == StructLib.SettlementType.FUTURE,
		...
		v.fee_A =
			int256(
				int128(
					ld._ledger[a.ledger_A].ft_feePerContract[a.tokTypeId] != 0
						? ld._ledger[a.ledger_A].ft_feePerContract[a.tokTypeId]
						: std._tt_ft[a.tokTypeId].feePerContract
				)
			) *
			v.posSize;
		v.fee_B =
			int256(
				int128(
					ld._ledger[a.ledger_B].ft_feePerContract[a.tokTypeId] != 0
						? ld._ledger[a.ledger_B].ft_feePerContract[a.tokTypeId]
						: std._tt_ft[a.tokTypeId].feePerContract
				)
			) *
			v.posSize;
		...
		require(
			StructLib.sufficientCcy(
				ld,
				a.ledger_A,
				std._tt_ft[a.tokTypeId].refCcyId,
				0,
				0,
				v.fee_A
			),
		...
		StructLib.transferCcy(
			...
			StructLib.TransferCcyArgs({
				from: a.ledger_A,
				to: owner,
				ccyTypeId: std._tt_ft[a.tokTypeId].refCcyId,
				amount: uint256(v.fee_A),
		...
		if (a.qty_A > 0)
			emit FutureOpenInterest(
				a.ledger_A,
				a.ledger_B,
				ld._tokens_currentMax_id - 1,
				a.tokTypeId,
				uint256(a.qty_A),
				uint256(a.price),
				uint256(v.fee_A),
	...
	function takePay2(
		...
		v = TakePayVars2({
			st: st,
			delta: delta,
			bal: ld._ledger[st.ft_ledgerOwner].ccyType_balance[fta.refCcyId],
			fee: 0,
			take: 0
		});
		...
		if (v.delta < 0) {
			v.take = v.delta * -1;
			if (v.take > v.bal) {
				// *capped* OTM take at physical balance
				v.take = v.bal;
			}
			...
			StructLib.emitTransferedLedgerCcy(
				StructLib.TransferCcyArgs({
					from: v.st.ft_ledgerOwner,
					to: a.feeAddrOwner,
					ccyTypeId: fta.refCcyId,
					amount: uint256(v.take),
					transferType: StructLib.TransferType.SettleTake
				})
			);
			...
			StructLib.emitTransferedLedgerCcy(
				StructLib.TransferCcyArgs({
					...
					to: a.feeAddrOwner,
					ccyTypeId: fta.refCcyId,
					amount: uint256(v.fee),
					transferType: StructLib.TransferType.TakePayFee
				})
			);
		} else if (v.delta > 0) {
			// *uncapped* ITM pay
			...
			require(
				ld._ledger[a.feeAddrOwner].ccyType_balance[fta.refCcyId] >= v.take,
				"Central insufficient for settlement"
			);
			...
			StructLib.emitTransferedLedgerCcy(
				StructLib.TransferCcyArgs({
					from: a.feeAddrOwner,
					to: v.st.ft_ledgerOwner,
					ccyTypeId: fta.refCcyId,
					amount: uint256(v.take),
					transferType: StructLib.TransferType.SettlePay
				})
			);
	...
	function combineFtPos(
		...
		require(
			masterSt.batchId == 0 && masterSt.ft_price != 0,
			"Bad (unexpected data) on master token"
		);
		require(masterSt.ft_lastMarkPrice != -1, "Bad last mark price on master token");
		require(
			StructLib.tokenExistsOnLedger(ld, a.tokTypeId, masterSt.ft_ledgerOwner, a.master_StId),
			"Bad or missing ledger token type on master token"
		);
		...
		for (uint256 x = 0; x < a.child_StIds.length; x++) {
			...
			require(
				childSt.batchId == 0 && childSt.ft_price != 0,
				"Bad (unexpected data) on child token"
			);
			require(
				childSt.ft_ledgerOwner == masterSt.ft_ledgerOwner,
				"Token ledger owner mismatch"
			);
			...
			require(
				StructLib.tokenExistsOnLedger(
					ld,
					a.tokTypeId,
					masterSt.ft_ledgerOwner,
					a.child_StIds[x]
				),
				"Bad or missing ledger token type on child token"
			);
			require(
				childSt.ft_lastMarkPrice == masterSt.ft_lastMarkPrice,
				"Last mark price mismatch"
			);
	...
	function setReservedAllFtPos(
		...
		int256[32 + 1] /*MAX_CCYS*/
			memory ccyId_newRes; // 1-based
		...
		for (uint256 tokTypeId = 1; tokTypeId <= std._tt_Count; tokTypeId++) {
			if (std._tt_settle[tokTypeId] == StructLib.SettlementType.FUTURE) {
				...
				for (
					uint256 x = 0;
					x < ld._ledger[ledgerOwner].tokenType_stIds[tokTypeId].length;
					x++
				) {
				...
				int256 totMargin = calcPosMargin(
					ld,
					std,
					ledgerOwner,
					tokTypeId,
	...
	function calcPosMargin(
		...
		uint16 totMargin = (
			ld._ledger[ledgerOwner].ft_initMarginBips[tokTypeId] != 0
		...
		return
			(((int256(int16(totMargin)) * 1000000) / 10000) * /*increase precision*/ /*basis points*/
				(int256(int16(std._tt_ft[tokTypeId].contractSize)) *
					...
					price)) / 1000000; /*notional*/ /*decrease precision*/
	}
	...
	function calcTakePay(
		...
		int256 delta = int256(
			markPrice - (ft_lastMarkPrice == -1 ? st.ft_price : ft_lastMarkPrice)
		) *
			int256(int16(fta.contractSize)) *
			int256(st.currentQty);
		return delta;
	}
	...
	function abs256(int256 x) private pure returns (int256) {
		return x < 0 ? x * -1 : x;
	}

	function abs64(int64 x) private pure returns (int64) {
		return x < 0 ? x * -1 : x;
	}
}
