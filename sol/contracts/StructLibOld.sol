...
library StructLibOld {
	...
	struct Ledger {
		...
		FeeStruct spot_customFees; // global fee override - per ledger entry
		uint256 spot_sumQtyMinted;
		uint256 spot_sumQtyBurned;
	...
	function transferCcy(LedgerStruct storage ld, TransferCcyArgs memory a) public { }
	

	function emitTransferedLedgerCcy(TransferCcyArgs memory a) public { }

	function setReservedCcy(
		LedgerStruct storage ld,
		CcyTypesStruct storage ctd,
		address ledger,
		uint256 ccyTypeId,
		int256 reservedAmount
	) public { }

	function initLedgerIfNew(LedgerStruct storage ld, address addr) public { }
