...
library TransferLib {
	...
	function transferOrTrade(
		...
		require(
			a.qty_A > 0 || a.qty_B > 0 || a.ccy_amount_A > 0 || a.ccy_amount_B > 0,
			"Bad null transfer"
		);
		...
		require(
			!((a.qty_A > 0 && a.ccy_amount_A > 0) || (a.qty_B > 0 && a.ccy_amount_B > 0)),
			"Bad transfer types"
		);
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_BASE) {
			if (a.ccy_amount_A > 0)
				require(a.ccyTypeId_A > 0 && a.ccyTypeId_A <= ctd._ct_Count, "Bad ccyTypeId A");
			if (a.ccy_amount_B > 0)
				require(a.ccyTypeId_B > 0 && a.ccyTypeId_B <= ctd._ct_Count, "Bad ccyTypeId B");
		}
		...
		if (
			(a.ccyTypeId_A > 0 && a.tokTypeId_B == 0) ||
			(a.ccyTypeId_B > 0 && a.tokTypeId_A == 0) ||
			(a.tokTypeId_A > 0 && a.ccyTypeId_B == 0 && a.tokTypeId_B == 0) ||
			(a.tokTypeId_B > 0 && a.ccyTypeId_A == 0 && a.tokTypeId_A == 0)
		) {
		...
		if (ld.contractType == StructLib.ContractType.CASHFLOW_CONTROLLER) {
			//**
			if (a.qty_A > 0) {
				StructLib.TransferArgs memory args = StructLib.TransferArgs({
					ledger_A: a.ledger_A,
					ledger_B: a.ledger_B,
					qty_A: a.qty_A,
		...
		if(customFee.applyCustomFee) {
			exFees.fee_ccy_A = customFee.feeA;
			exFees.fee_ccy_B = customFee.feeB;
		} else {
			...
			exFees.fee_ccy_B = a.ledger_B != a.feeAddrOwner_B
				? calcFeeWithCapCollar(exFeeStruct_ccy_B.ccy[a.ccyTypeId_B], uint256(a.ccy_amount_B), a.qty_A): 0;

			if (exFees.fee_ccy_A > 0 && exFees.fee_ccy_B == 0 && exFeeStruct_ccy_A.ccy[a.ccyTypeId_A].ccy_mirrorFee == true) {
				...
				exFees.fee_ccy_B = a.ledger_B != a.feeAddrOwner_B
					...
					: 0; // ??!
			} else if (exFees.fee_ccy_B > 0 && exFees.fee_ccy_A == 0 && exFeeStruct_ccy_B.ccy[a.ccyTypeId_B].ccy_mirrorFee == true) {
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_CONTROLLER) {
			//**
			if (a.qty_A > 0) {
				v.ts_args[0] = TransferSplitArgs({
					from: a.ledger_A,
					to: a.ledger_B,
					tokTypeId: a.tokTypeId_A,
					qtyUnit: a.qty_A,
					transferType: a.transferType == StructLib.TransferType.Undefined
						? StructLib.TransferType.User
						: a.transferType,
					maxStId: maxStId,
					k_stIds_take: a.k_stIds_A /*, k_stIds_skip: new uint256[](0)*/
				});
				v.ts_previews[0] = transferSplitSecTokens_Preview(ld, v.ts_args[0]);
				for (uint256 i = 0; i < v.ts_previews[0].batchCount; i++) {
					StructLib.SecTokenBatch storage batch = ld._batches[
						v.ts_previews[0].batchIds[i]
					];

					v.totalOrigFee[0] += a.ledger_A != batch.originator
						? calcFeeWithCapCollar(batch.origTokFee, v.ts_previews[0].transferQty[i], 0)
						: 0;
				}
			}
			if (a.qty_B > 0) {
				v.ts_args[1] = TransferSplitArgs({
					from: a.ledger_B,
					to: a.ledger_A,
				...
				for (uint256 i = 0; i < v.ts_previews[1].batchCount; i++) {
					StructLib.SecTokenBatch storage batch = ld._batches[
						v.ts_previews[1].batchIds[i]
					];

					v.totalOrigFee[1] += a.ledger_B != batch.originator
						? calcFeeWithCapCollar(batch.origTokFee, v.ts_previews[1].transferQty[i], 0)
						: 0;
				}
			}
		}
			...
			require(
				StructLib.sufficientCcy(
					...
					int256(
						exFees.fee_ccy_A *
							uint256(
								a.applyFees /*&& a.ccy_amount_A > 0 */
									? 1
									: 0
							)
					)
				),
				"Insufficient currency A"
			);
			require(
				StructLib.sufficientCcy(
					ld,
					...
					int256(
						exFees.fee_ccy_B *
							uint256(
								a.applyFees /*&& a.ccy_amount_B > 0 */
									? 1
									: 0
							)
					)
				),
				"Insufficient currency B"
			);
		}
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_CONTROLLER) {
			...
			require(
				StructLib.sufficientTokens(
					...
					int256(a.qty_A),
					int256(
						(exFees.fee_tok_A + v.totalOrigFee[0]) *
							(a.applyFees && a.qty_A > 0 ? 1 : 0)
					)
				),
				"Insufficient tokens A"
			);
			require(
				StructLib.sufficientTokens(
					ld,
					...
					int256(a.qty_B),
					int256(
						(exFees.fee_tok_B + v.totalOrigFee[1]) *
							(a.applyFees && a.qty_B > 0 ? 1 : 0)
					)
				),
				"Insufficient tokens B"
			);
		}
			...
			if (a.applyFees && a.transferType == StructLib.TransferType.Undefined && ((a.qty_A > 0 && a.ccy_amount_B > 0) || (a.qty_B > 0 && a.ccy_amount_A > 0))) {
				if (exFees.fee_ccy_A + exFees.fee_ccy_B > 0) {
					...
					if(a.feeAddrOwner_A == a.feeAddrOwner_B) {
						...
						applyOriginatorCcyFees(ld, v.ts_previews[0], exFees.fee_ccy_A + exFees.fee_ccy_B, a.qty_A, a.feeAddrOwner_A, ccyTypeId);

						// apply for B->A token batches
						applyOriginatorCcyFees(ld, v.ts_previews[1], exFees.fee_ccy_A + exFees.fee_ccy_B, a.qty_B, a.feeAddrOwner_A, ccyTypeId);
					} else {
						if(exFees.fee_ccy_A > 0) {
							applyOriginatorCcyFees(ld, v.ts_previews[0], exFees.fee_ccy_A, a.qty_A, a.feeAddrOwner_A, ccyTypeId);
							applyOriginatorCcyFees(ld, v.ts_previews[1], exFees.fee_ccy_A, a.qty_B, a.feeAddrOwner_A, ccyTypeId);
						}

						if(exFees.fee_ccy_B > 0) {
							applyOriginatorCcyFees(ld, v.ts_previews[0], exFees.fee_ccy_B, a.qty_A, a.feeAddrOwner_B, ccyTypeId);
							applyOriginatorCcyFees(ld, v.ts_previews[1], exFees.fee_ccy_B, a.qty_B, a.feeAddrOwner_B, ccyTypeId);
						}
					}
				}
			}
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_CONTROLLER) {
			...
			if (a.qty_A > 0) {
				if (a.applyFees) {
					...
					if (exFees.fee_tok_A > 0) {
						maxStId = transferSplitSecTokens(
							ld,
							TransferSplitArgs({
								from: a.ledger_A,
								to: a.feeAddrOwner_A,
					...
					for (uint256 i = 0; i < v.ts_previews[0].batchCount; i++) {
						// originator token fees
						maxStId = batchOrigTokFeeTransferA(ld, a, v, maxStId, i);
					}
				}
				// user transfer from A
				maxStId = transferSplitSecTokens(
					ld,
					TransferSplitArgs({
						from: v.ts_args[0].from,
						to: v.ts_args[0].to,
						tokTypeId: v.ts_args[0].tokTypeId,
						qtyUnit: v.ts_args[0].qtyUnit,
						transferType: v.ts_args[0].transferType,
						maxStId: maxStId,
						k_stIds_take: a.k_stIds_A /*, k_stIds_skip: new uint256[](0)*/
					})
				);
			...
			if (a.qty_B > 0) {
				if (a.applyFees) {
					...
					for (uint256 i = 0; i < v.ts_previews[1].batchCount; i++) {
						// originator token fees
						maxStId = batchOrigTokFeeTransferB(ld, a, v, maxStId, i);
					}
				}
				// user transfer from B
				maxStId = transferSplitSecTokens(
					ld,
					TransferSplitArgs({
						from: v.ts_args[1].from,
						to: v.ts_args[1].to,
						tokTypeId: v.ts_args[1].tokTypeId,
						qtyUnit: v.ts_args[1].qtyUnit,
						transferType: v.ts_args[1].transferType,
						maxStId: maxStId,
						k_stIds_take: a.k_stIds_B /*, k_stIds_skip: new uint256[](0)*/
					})
				);
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_BASE) {
			...
			if (a.ccy_amount_A > 0 && a.qty_B > 0) {
				emitTradedCcyTokA(a, exFees, customFee.applyCustomFee);
			}
			if (a.ccy_amount_B > 0 && a.qty_A > 0) {
				emitTradedCcyTokB(a, exFees, customFee.applyCustomFee);
			}
		}
	}

	function emitTradedCcyTokA(
		StructLib.TransferArgs memory a,
		StructLib.FeesCalc memory exFees,
		bool isFeeCustom
	) internal {
		emit TradedCcyTok(
			a.ccyTypeId_A,
			...
			a.ledger_A,
			a.qty_B,
			a.applyFees ? exFees.fee_ccy_B : 0,
			a.applyFees ? exFees.fee_ccy_A : 0,
			isFeeCustom
		);
	}

	function emitTradedCcyTokB(
		StructLib.TransferArgs memory a,
		StructLib.FeesCalc memory exFees,
		bool isFeeCustom
	) internal {
	...
	function batchOrigTokFeeTransferA(
		StructLib.LedgerStruct storage ld,
		StructLib.TransferArgs memory a,
		TransferVars memory v,
		uint maxStId,
		...
		uint256 tokFee = a.ledger_A != batch.originator
			? calcFeeWithCapCollar(
				batch.origTokFee,
				v.ts_previews[0].transferQty[i],
				0
			)
			: 0;
		if (tokFee > 0) {
			maxStId = transferSplitSecTokens(
				...
				TransferSplitArgs({
					...
					tokTypeId: a.tokTypeId_A,
					qtyUnit: tokFee,
					transferType: StructLib.TransferType.OriginatorFee,
					maxStId: maxStId,
					k_stIds_take: a.k_stIds_A /*, k_stIds_skip: new uint256[](0)*/
				})
			);
	...
	function batchOrigTokFeeTransferB(
		StructLib.LedgerStruct storage ld,
		StructLib.TransferArgs memory a,
		TransferVars memory v,
		uint maxStId,
		...
		uint256 tokFee = a.ledger_B != batch.originator
			? calcFeeWithCapCollar(
				batch.origTokFee,
				v.ts_previews[1].transferQty[i],
				0
			)
			: 0;
		if (tokFee > 0) {
	...
	function applyOriginatorCcyFees(
		...
		for (uint256 i = 0; i < ts_preview.batchCount; i++) {
		...
		for (uint256 i = 0; i < ts_preview.batchCount; i++) {
			...
			uint256 batch_exFee_ccy = (((ts_preview.transferQty[i] * 1000000) / tot_qty) *
				tot_exFee_ccy) / 1000000; /*increase precision*/ /*decrease precision*/

			// batch fee - capped share of exchange ccy fee
			uint256 BFEE = (((uint256(batch.origCcyFee_percBips_ExFee) * 1000000) / 10000) *
				batch_exFee_ccy) / 1000000; /*increase precision*/ /*basis points*/ /*decrease precision*/
	...
	function transferSplitSecTokens(StructLib.LedgerStruct storage ld, TransferSplitArgs memory a)
		internal
		returns (uint256 updatedMaxStId)
	{
		...
		while (v.remainingToTransfer > 0) {
			...
			if (a.k_stIds_take.length > 0) {
				...
				for (uint256 i = 0; i < a.k_stIds_take.length; i++) {
					if (a.k_stIds_take[i] == stId) {
						skip = false;
						break;
					} // i.e. take wins over skip (if same STID is specified in both take and skip list)
				}
			}
			if (skip) {
				v.ndx++;
				...
				if (v.remainingToTransfer >= v.stQty) {
					...
					for (uint256 i = 0; i < to_stIds.length; i++) {
						if (ld._sts[to_stIds[i]].batchId == ld._sts[stId].batchId) {
							...
							emit TransferedFullSecToken(
								a.from,
								a.to,
								stId,
								to_stIds[i],
								uint256(uint64(v.stQty)),
								a.transferType
							);
							break;
						}
					...
					// move PART of an ST across the ledger entries
					...
					for (uint256 i = 0; i < to_stIds.length; i++) {
						if (ld._sts[to_stIds[i]].batchId == ld._sts[stId].batchId) {
							...
							v.mergedExisting = true;
							emit TransferedPartialSecToken(
								a.from,
								a.to,
								stId,
								0,
								to_stIds[i],
								uint256(uint64(v.remainingToTransfer)),
								a.transferType
							);
							break;
						}
	...
	function transferSplitSecTokens_Preview(
	...
	) internal view returns (TransferSplitPreviewReturn memory ret) {
		...
		while (remainingToTransfer > 0) {
			...
			if (skip) {
				...
				for (uint256 i = 0; i < ret.batchCount; i++) {
					if (ret.batchIds[i] == fromBatchId) {
						ret.transferQty[i] += uint256(
				...
				if (!knownBatch) {
					require(
						ret.batchCount < MAX_BATCHES_PREVIEW,
						"Too many batches: try sending a smaller amount"
					);
					ret.batchIds[ret.batchCount] = fromBatchId;
					ret.transferQty[ret.batchCount] = uint256(
						remainingToTransfer >= stQty ? uint64(stQty) : uint64(remainingToTransfer)
					);
					ret.batchCount++;
				}
				if (remainingToTransfer >= stQty) {
					// full ST transfer, and more needed
					...
					// partial ST transfer, and no more needed
	...
	function applyFeeStruct(
		...
		uint256 feeAmount = fs.fee_fixed +
			((((receiveAmount * 1000000000) / 1000000) * fs.ccy_perMillion) / 1000000000) + /*increase precision*/ /*per million*/ /*decrease precision*/
			((((sendAmount * 1000000) / 10000) * fs.fee_percBips) / 1000000); /*increase precision*/ /*basis points*/ /*decrease precision*/
		if (sendAmount > 0) {
	...
	//
	function checkStIds(StructLib.LedgerStruct storage ld, StructLib.TransferArgs memory a)
		internal
		view
	{
		if (a.k_stIds_A.length > 0) {
			...
			for (uint256 i = 0; i < a.k_stIds_A.length; i++) {
				require(
					StructLib.tokenExistsOnLedger(ld, a.tokTypeId_A, a.ledger_A, a.k_stIds_A[i]),
					"Bad stId A"
				);
		...
		if (a.k_stIds_B.length > 0) {
			...
			for (uint256 i = 0; i < a.k_stIds_B.length; i++) {
				require(
					StructLib.tokenExistsOnLedger(ld, a.tokTypeId_B, a.ledger_B, a.k_stIds_B[i]),
					"Bad stId B"
				);
