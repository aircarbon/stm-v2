...
library TransferLib {
	...
	//
	{
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_CONTROLLER) {
			...
			uint256 maxStId = ld._tokens_currentMax_id;
			if (a.qty_A > 0) {
				TransferSplitPreviewReturn memory preview = transferSplitSecTokens_Preview(
					ld,
					TransferSplitArgs({
						from: a.ledger_A,
						to: a.ledger_B,
				...
				for (uint256 i = 0; i < preview.batchCount; i++) {
					...
					if (a.ledger_A != batch.originator) {
						feesAll[ndx++] = StructLib.FeesCalc({
							fee_ccy_A: 0,
							fee_ccy_B: 0,
							fee_tok_A: calcFeeWithCapCollar(
								batch.origTokFee,
				...
				for (uint256 i = 0; i < preview.batchCount; i++) {
					...
					if (a.ledger_B != batch.originator) {
						feesAll[ndx++] = StructLib.FeesCalc({
							...
							fee_ccy_B: 0,
							fee_tok_A: 0,
							fee_tok_B: calcFeeWithCapCollar(
								batch.origTokFee,
								preview.transferQty[i],
								0
							),
