...
library LedgerLib {
	...
	function getLedgerEntry(
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_CONTROLLER) {
			for (uint256 tokTypeId = 1; tokTypeId <= std._tt_Count; tokTypeId++) {
				// get ST IDs & data from local storage
		...
		// controller: get STs from base ledger types (and sum total spot sizes across all types)
			...
			for (uint256 tokTypeId = 1; tokTypeId <= std._tt_Count; tokTypeId++) {
				// get ST IDs & data from base types' storage
	...
	function getLedgerHashcode(
		...
		for (uint256 ccyTypeId = 1; ccyTypeId <= ctd._ct_Count; ccyTypeId++) {
			...
			ledgerHash = keccak256(
				abi.encodePacked(ledgerHash, ccy.id, ccy.name, ccy.unit, ccy.decimals)
			);

			if (globalFees.ccyType_Set[ccyTypeId]) {
				ledgerHash = keccak256(
					abi.encodePacked(ledgerHash, hashSetFeeArgs(globalFees.ccy[ccyTypeId]))
				);
			}
		}
		...
		for (uint256 stTypeId = 1; stTypeId <= currentMaxTokenTypeId; stTypeId++) {
			...
			ledgerHash = keccak256(
				abi.encodePacked(ledgerHash, std._tt_ft[stTypeId].expiryTimestamp)
			);
			ledgerHash = keccak256(
				abi.encodePacked(ledgerHash, std._tt_ft[stTypeId].underlyerTypeId)
			);
			ledgerHash = keccak256(abi.encodePacked(ledgerHash, std._tt_ft[stTypeId].refCcyId));
			ledgerHash = keccak256(
				abi.encodePacked(ledgerHash, std._tt_ft[stTypeId].initMarginBips)
			);
			ledgerHash = keccak256(
				abi.encodePacked(ledgerHash, std._tt_ft[stTypeId].varMarginBips)
			);
			ledgerHash = keccak256(abi.encodePacked(ledgerHash, std._tt_ft[stTypeId].contractSize));
			ledgerHash = keccak256(
				abi.encodePacked(ledgerHash, std._tt_ft[stTypeId].feePerContract)
			);
			...
			if (globalFees.tokType_Set[stTypeId]) {
				ledgerHash = keccak256(
					abi.encodePacked(ledgerHash, hashSetFeeArgs(globalFees.tok[stTypeId]))
				);
			}
		}
		...
		for (
			uint256 whitelistNdx = 1; // exclude contract owner @ndx 0
			whitelistNdx < whiteListLength;
			whitelistNdx++
		) {
			if (whitelistNdx % mod != n) continue;

			if (
				erc20d._whitelist[whitelistNdx] != msg.sender // exclude caller, contract owner
			) {
				ledgerHash = keccak256(
					abi.encodePacked(ledgerHash, erc20d._whitelist[whitelistNdx])
				);
			}
		}
		...
		for (uint256 batchId = 1; batchId <= maxCurrentBatchId; batchId++) {
			...
			if (batch.originator != msg.sender) {
		...
		for (uint256 ledgerNdx = 0; ledgerNdx < ledgerOwnersCount; ledgerNdx++) {
			...
			if (ledgerNdx != 0) ledgerHash = keccak256(abi.encodePacked(ledgerHash, entryOwner));
			...
			for (uint256 stTypeId = 1; stTypeId <= currentMaxTokenTypeId; stTypeId++) {
				...
				if (entry.spot_customFees.tokType_Set[stTypeId]) {
					ledgerHash = keccak256(
						abi.encodePacked(
							ledgerHash,
							hashSetFeeArgs(entry.spot_customFees.tok[stTypeId])
						)
					);
				}
				ledgerHash = keccak256(
					abi.encodePacked(ledgerHash, entry.ft_initMarginBips[stTypeId])
				);
				ledgerHash = keccak256(
					abi.encodePacked(ledgerHash, entry.ft_feePerContract[stTypeId])
				);
			...
			for (uint256 ccyTypeId = 1; ccyTypeId <= currentMaxCcyTypeId; ccyTypeId++) {
				ledgerHash = keccak256(
					abi.encodePacked(ledgerHash, entry.ccyType_balance[ccyTypeId])
				);
				ledgerHash = keccak256(
					abi.encodePacked(ledgerHash, entry.ccyType_reserved[ccyTypeId])
				);
				if (entry.spot_customFees.ccyType_Set[ccyTypeId]) {
					ledgerHash = keccak256(
						abi.encodePacked(
							ledgerHash,
							hashSetFeeArgs(entry.spot_customFees.ccy[ccyTypeId])
						)
					);
				}
			}
		...
		if (ld.contractType == StructLib.ContractType.CASHFLOW_CONTROLLER) {
			...
			for (uint256 tokTypeId = 1; tokTypeId <= currentMaxTokenTypeId; tokTypeId++) {
				StTransferableFacet base = StTransferableFacet(std._tt_addr[tokTypeId]);
		...
		if (ld.contractType != StructLib.ContractType.CASHFLOW_CONTROLLER) {
			...
			require(
				chk.totalMinted - chk.totalCur == ld._spot_totalBurnedQty,
				"Consistency check failed (2)"
			);
		}
	...
	function hashSetFeeArgs(StructLib.SetFeeArgs memory setFeeArgs)
		private
		pure
		returns (bytes32 setFeeArgsHash)
	{
		setFeeArgsHash = keccak256(
			abi.encodePacked(
