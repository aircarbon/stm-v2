...
contract StFeesFacet {
	...
	function getFee(
		...
		if(ledgerOwner == address(0x0)) {
			...
			fs = s.entityGlobalFees[entityId];
		} else {
	...
	function setFee_TokType(
		...
		LibMainStorage.MainStorage storage s = LibMainStorage.getStorage();
		SpotFeeLib.setFee_TokType(s.ld, s.std, s.entityGlobalFees, entityId, tokTypeId, ledgerOwner, feeArgs);
	}
	...
	function setFee_TokTypeBatch(
		...
		LibMainStorage.MainStorage storage s = LibMainStorage.getStorage();
		SpotFeeLib.setFee_TokTypeBatch(s.ld, s.std, s.entityGlobalFees, entityId, tokTypeId, ledgerOwner, feeArgs);
	}
	...
	function setFee_CcyType(
		...
		LibMainStorage.MainStorage storage s = LibMainStorage.getStorage();
		SpotFeeLib.setFee_CcyType(s.ld, s.ctd, s.entityGlobalFees, entityId, ccyTypeId, ledgerOwner, feeArgs);
	}
	...
	function setFee_CcyTypeBatch(StructLib.SetFeeCcyTypeBatchArgs[] calldata params) external {
		...
		LibMainStorage.MainStorage storage s = LibMainStorage.getStorage();
		SpotFeeLib.setFee_CcyTypeBatch(s.ld, s.ctd, s.entityGlobalFees, params);
	}
}
