...
contract("DiamondProxy", accounts => {
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply NATURE token ledger override fee 1000 BP + 5 TONS fixed (cap 10 TONS) on a small trade (fee on A)`, async () => {
        ...
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, CONST.nullAddr,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );

        // set ledger fee structure NATURE for A: 10% + 5 TONS, CAP 10 TONS
        const feeBps = 1000; 
        const feeFix = 5;
        const feeCap = 10;
        const setEeuFeeTx = await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, A, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeCap } );
        ...
        assert(Big(owner_balAfter).eq(Big(owner_balBefore).plus(Big(expectedFeeTokQty))), 'unexpected fee receiver A token balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply then clear NATURE token ledger override fee 1000 BP + 5 TONS fixed (cap 10 TONS) on a small trade (fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.KT_CARBON, 1,      A, CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,        CONST.oneEth_wei,  B, 'TEST');
        ...
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: 0, fee_max: globalFeeCap } );
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, CONST.nullAddr,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0, fee_max: 0 } );
        ...
        var feeFix = 5;
        var feeCap = 10;
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, A, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeCap } );

        // clear ledger fee NATURE for A (zero)
        feeBps = 0;
        feeFix = 0;
        feeCap = 0;
        const clearEeuFeeTx = await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, A, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeCap } );
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply NATURE token ledger override fee 1000 BP + 1000 tons fixed (collar 100m tons), on a large (0.5 GT) trade (fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];

        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,        CONST.oneEth_wei,        A, 'TEST');
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.GT_CARBON, 1,       B, CONST.nullFees, 0, [], [], { from: accounts[0] });

        // set global fee structure (zero)
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
        await stmStFeesFacet.setFee_CcyType(1, CONST.ccyType.ETH, CONST.nullAddr,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
        ...
        const setEeuFeeTx = await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        const setCcyFeeTx = await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, B,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,      fee_percBips: 0,      fee_min: 0,      fee_max: 0 } );
        ...
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T1, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix+1, fee_percBips: feeBps+1, fee_min: feeMin+1, fee_max: 0 } );
        ...
        let owner_balBefore = data.feeOwnerLedgerForA_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        let owner_balAfter  =  data.feeOwnerLedgerForA_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfter).eq(Big(owner_balBefore)), 'unexpected fee receiver A token balance after transfer');

        owner_balBefore = data.feeOwnerLedgerForB_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balAfter  =  data.feeOwnerLedgerForB_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfter).eq(Big(owner_balBefore).plus(Big(expectedFeeTokQty))), 'unexpected fee receiver B token balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply then clear NATURE token ledger override fee 1000 BP + 1000 tons fixed (collar 100m tons), on a large (0.5 GT) trade (fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        ...
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: globalFeeMin, fee_max: 0 } );
        await stmStFeesFacet.setFee_CcyType(1, CONST.ccyType.ETH, CONST.nullAddr,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0,            fee_max: 0 } );
        ...
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, B,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,      fee_percBips: 0,      fee_min: 0,      fee_max: 0 } );

        // clear ledger fee structure on B
        feeBps = 0;
        feeFix = 0;
        feeMin = 0;
        const clearEeuFeeTx = await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply ETH ccy override fee 2500 BP + 0.01 ETH fixed (collar 0.2 ETH), on a small trade (fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.oneEth_wei,        A, 'TEST');
        ...
        const setCcyFeeTx = await stmStFeesFacet.setFee_CcyType(1, CONST.ccyType.ETH, A,   { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: ethFeeMin, fee_max: 0 } );
        ...
        assert(Big(owner_balAfter).eq(Big(owner_balBefore).plus(Big(expectedFeeCcy))), 'unexpected fee receiver A currency balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply then clear ETH ccy override fee 2500 BP + 0.01 ETH fixed (collar 0.2 ETH), on a small trade (fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.oneEth_wei,        A, 'TEST');
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.KT_CARBON, 1,      B, CONST.nullFees, 0, [], [], { from: accounts[0] });
        ...
        await stmStFeesFacet.setFee_CcyType(1, CONST.ccyType.ETH, CONST.nullAddr,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: globalFeeMin, fee_max: 0 } );
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0,            fee_max: 0 } );
        ...
        await stmStFeesFacet.setFee_CcyType(1, CONST.ccyType.ETH, A,   { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: ethFeeMin, fee_max: 0 } );

        // clear ledger fee ETH for A (zero)
        ethFeeBps = 0;
        ethFeeFix = 0;
        ethFeeMin = 0;
        const clearFeeTx = await stmStFeesFacet.setFee_CcyType(1, CONST.ccyType.ETH, A, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: ethFeeMin, fee_max: 0 } );
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply ETH ccy ledger override fee 1000 BP + 1000 ETH fixed (cap 50000 ETH), on a large (500k ETH) trade (fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];

        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.KT_CARBON, 1,      A, CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.millionEth_wei,    B, 'TEST');
        ...
        const setCcyFeeTx = await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, B,   { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: ethFeeMax } );
        const setEeuFeeTx = await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,         fee_percBips: 0,         fee_min: 0, fee_max: 0 } );
        ...
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.USD, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ethFeeFix+1, fee_percBips: ethFeeBps+1, fee_min: 0, fee_max: 0 } );
        ...
        owner_balBefore = data.feeOwnerLedgerForB_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH).map(p => p.balance).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balAfter  =  data.feeOwnerLedgerForB_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH).map(p => p.balance).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfter).eq(Big(owner_balBefore).plus(Big(expectedFeeCcy))), 'unexpected fee receiver B currency balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - apply then clear ETH ccy ledger override fee 1000 BP + 1000 ETH fixed (cap 50000 ETH), on a large (250k ETH) trade (fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        ...
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, CONST.nullAddr,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: 0, fee_max: globalFeeMax } );
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0, fee_max: 0 } );
        ...
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, B,      { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: ethFeeMax } );
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0,         fee_percBips: 0,         fee_min: 0, fee_max: 0 } );

        // clear ledger fee structure (zero) on B
        ethFeeBps = 0;
        ethFeeFix = 0;
        ethFeeMax = 0;
        const clearFeeTx = await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: ethFeeMax } );
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - should allow a ledger fee-capped transfer from A with otherwise insufficient carbon to cover fees (ledger fee on A, global fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        ...
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, A, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ledgerFeeFix, fee_percBips: ledgerFeeBps, fee_min: 0, fee_max: ledgerFeeMax } );
        ...
        // feeOwnerLedgerForA != feeOwnerLedgerForB
        let owner_balBeforeTokQty = data.feeOwnerLedgerForA_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        let owner_balAfterTokQty  =  data.feeOwnerLedgerForA_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfterTokQty).eq(Big(owner_balBeforeTokQty).plus(Big(expectedFeeTokQty))), 'unexpected fee receiver A token balance after transfer');

        owner_balBeforeTokQty = data.feeOwnerLedgerForB_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balAfterTokQty  =  data.feeOwnerLedgerForB_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfterTokQty).eq(Big(owner_balBeforeTokQty)), 'unexpected fee receiver B token balance after transfer');
        ...
        // contract owner has received expected ccy fee
        // feeOwnerLedgerForA != feeOwnerLedgerForB
        let owner_balBeforeCcy = data.feeOwnerLedgerForA_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH).map(p => p.balance).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        let owner_balAfterCcy  =  data.feeOwnerLedgerForA_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH).map(p => p.balance).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfterCcy).eq(Big(owner_balBeforeCcy)), 'unexpected fee receiver A currency balance after transfer');

        owner_balBeforeCcy = data.feeOwnerLedgerForB_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH).map(p => p.balance).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balAfterCcy  =  data.feeOwnerLedgerForB_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH).map(p => p.balance).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfterCcy).eq(Big(owner_balBeforeCcy).plus(Big(expectedFeeEth))), 'unexpected fee receiver B currency balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - should allow a ledger fee-capped transfer from B with otherwise insufficient ccy to cover fees (ledger fee on B, global fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        ...
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: 0, fee_max: globalFeeMax } );

        // set ledger fee structure ETH (B)
        const ledgerFeeFix = 0;
        const ledgerFeeBps = 2000; // 20%
        const ledgerFeeMax = CONST.tenthEth_wei;
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: ledgerFeeFix, fee_percBips: ledgerFeeBps, fee_min: 0, fee_max: ledgerFeeMax } );
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - should not allow a transfer with insufficient carbon to cover collared ledger fee (fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
    ...
    it(`cross-entity transfer (different fee owners) - fees (ledger) - should not allow a transfer with insufficient carbon to cover collared ledger fee (fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        ...
        const feeFix = 1000;
        const feeMin = 101000;
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, A, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH,      B,              CONST.nullFees);
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.ETH,      CONST.nullAddr, CONST.nullFees);
