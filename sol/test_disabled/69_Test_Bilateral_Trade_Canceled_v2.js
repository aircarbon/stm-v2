...
contract("DiamondProxy", accounts => {
    ...
    it(`Bilateral Trade - cancel - should fail to request trade from non-owner`, async () => {
        ...
        try {
            await stmStTransferableFacet.cancelBilateralTrade(0, referenceTx, A, B, ccyTypeId, tokenTypeId, ccyQty, tokQty, metadata, { from: accounts[10] });
        } catch (ex) { assert(ex.reason == 'Restricted', `unexpected: ${ex.reason}`); return; }
        assert.fail('expected contract exception');
    });

    it(`Bilateral Trade - cancel - should fail to request trade when contract is read-only`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
        ...
        try {
            ...
            await stmStTransferableFacet.cancelBilateralTrade(0, referenceTx,  A, B, ccyTypeId, tokenTypeId, ccyQty, tokQty, metadata);
        } catch (ex) { 
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with user without entity (ledger A)`, async () => {
        const A = accounts[99];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with user without entity (ledger B)`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[99];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with zero address (ledger A)`, async () => {
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with zero address (ledger B)`, async () => {
        const A = accounts[global.TaddrNdx];
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with zero ccyTypeId`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = 0;
        const tokenTypeId = CONST.tokenType.TOK_T2;
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with invalid ccyTypeId`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = 99;
        const tokenTypeId = CONST.tokenType.TOK_T2;
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with zero tokenTypeId`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = 0;
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with invalid tokenTypeId`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with zero tokenQty`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
    ...
    it(`Bilateral Trade - cancel - should fail to request trade with zero reference tx`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
        ...
        try {
            ...
            assert(ex.reason == 'cancelBilateralTrade: invalid referenceTx', `unexpected: ${ex.reason}`); 
            return; 
        }
        assert.fail('expected contract exception');
    });

    it(`Bilateral Trade - cancel - successfully emit an event with negative ccyQty`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
        ...
        const tx = await stmStTransferableFacet.cancelBilateralTrade(1, referenceTx, A, B, ccyTypeId, tokenTypeId, ccyQty, tokQty, metadata);

        truffleAssert.eventEmitted(tx, 'bilateralTradeCancelled', ev => { 
    ...
    it(`Bilateral Trade - cancel - should successfully emit event with empty metadata`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
    ...
    it(`Bilateral Trade - cancel - should successfully emit event with non empty metadata`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
    ...
    it(`Bilateral Trade - cancel - should successfully emit event with another trade type`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
        ...
        const tx = await stmStTransferableFacet.cancelBilateralTrade(2, referenceTx, A, B, ccyTypeId, tokenTypeId, ccyQty, tokQty, metadata);
    ...
    it(`Bilateral Trade - cancel - should successfully emit event with another trade type (2)`, async () => {
        const A = accounts[global.TaddrNdx];
        const B = accounts[global.TaddrNdx + 1];
        const ccyTypeId = CONST.ccyType.USD;
        const tokenTypeId = CONST.tokenType.TOK_T2;
        ...
        const tx = await stmStTransferableFacet.cancelBilateralTrade(3, referenceTx, A, B, ccyTypeId, tokenTypeId, ccyQty, tokQty, metadata);
