...
contract("DiamondProxy", accounts => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi-capcol) - apply NATURE token fee 1000 BP + 5 TONS fixed (cap 10 TONS) on a small trade (fee on A)`, async () => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi-capcol) - apply NATURE token fee 1000 BP + 1000 tons fixed (collar 100m tons), on a large (0.5 GT) trade (fee on B)`, async () => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi-capcol) - apply ETH ccy fee 2500 BP + 0.01 ETH fixed (collar 0.2 ETH), on a small trade (fee on A)`, async () => {
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,     CONST.oneEth_wei,              accounts[global.TaddrNdx + 0], 'TEST', );
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi-capcol) - apply ETH ccy fee 1000 BP + 1000 ETH fixed (cap 50000 ETH), on a large (500k ETH) trade (fee on B)`, async () => {
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 0], CONST.nullFees, 0, [], [], { from: accounts[0] });
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi-capcol) - should allow a capped transfer with otherwise insufficient carbon to cover fees (fee on A)`, async () => {
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 0], CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.oneEth_wei,              accounts[global.TaddrNdx + 1], 'TEST', );
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi-capcol) - should not allow a transfer with insufficient currency to cover collared fees (fee on A)`, async () => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi-capcol) - should not allow a transfer with insufficient carbon to cover collared fees (fee on B)`, async () => {
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.oneEth_wei,              accounts[global.TaddrNdx + 0], 'TEST', );
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 1], CONST.nullFees, 0, [], [], { from: accounts[0] });
