...
contract("DiamondProxy", accounts => {
    ...
    // ST FEES
    it(`cross-entity transfer (different fee owners) - fees (percentage) - apply NATURE token fee 100 BP on a trade (fee on A)`, async () => {
        ...
        assert(contractOwner_VcsTokQtyAfter == Number(contractOwner_VcsTokQtyBefore) + Number(expectedFeeTokQty), 'unexpected contract owner (fee receiver A) NATURE ST quantity after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (percentage) - apply CORSIA token fee 1 BP (min) on a trade 1000 tons (min lot size) (fee on B)`, async () => {
        ...
        assert(contractOwnercorsiaTokQtyAfter == Number(contractOwnercorsiaTokQtyBefore), 'unexpected contract owner (fee receiver A) CORSIA ST quantity after transfer');
        ...
        assert(contractOwnercorsiaTokQtyAfter == Number(contractOwnercorsiaTokQtyBefore) + Number(expectedFeeTokQty), 'unexpected contract owner (fee receiver B) CORSIA ST quantity after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (percentage) - apply large (>1 batch ST size) token fee 5000 BP on a trade on a newly added ST type`, async () => {
        ...
        // feeOwnerLedgerForA != feeOwnerLedgerForB
        let owner_balBefore = data.feeOwnerLedgerForA_before.tokens.filter(p => p.tokTypeId == newTypeId).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        let owner_balAfter  =  data.feeOwnerLedgerForA_after.tokens.filter(p => p.tokTypeId == newTypeId).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        assert(owner_balAfter == Number(owner_balBefore) + Number(expectedFeeTokQty), 'unexpected contract owner (fee receiver A) new ST type quantity after transfer');

        owner_balBefore = data.feeOwnerLedgerForB_before.tokens.filter(p => p.tokTypeId == newTypeId).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        owner_balAfter  =  data.feeOwnerLedgerForB_after.tokens.filter(p => p.tokTypeId == newTypeId).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
    ...
    // CCY FEES
    it(`cross-entity transfer (different fee owners) - fees (percentage) - apply ETH ccy fee 100 BP on a trade (fee on A)`, async () => {
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,    CONST.oneEth_wei,              accounts[global.TaddrNdx + 0],  'TEST', );
        ...
        assert(owner_balAfter == Number(owner_balBefore) + Number(expectedFeeCcy), 'unexpected contract owner (fee receiver A) ETH balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (percentage) - apply USD ccy fee 1 BP on a trade (fee on B)`, async () => {
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,  CONST.KT_CARBON, 1, accounts[global.TaddrNdx + 0], CONST.nullFees, 0, [], [], { from: accounts[0] });
        ...
        assert(owner_balAfter == Number(owner_balBefore) + Number(expectedFeeCcy), 'unexpected contract owner (fee receiver B) USD balance after transfer');
    });

    it(`cross-entity transfer (different fee owners) - fees (percentage) - apply ccy fee 50 BP on a trade on a newly added ccy`, async () => {
        ...
        assert(owner_balAfter == Number(owner_balBefore) + Number(expectedFeeCcy), 'unexpected contract owner (fee receiver B) new ccy balance after transfer');
    });

    // ST + CCY FEES
    it(`cross-entity transfer (different fee owners) - fees (percentage) - apply ETH ccy & NATURE ST fee on a 0.5 ST trade (fees on both sides)`, async () => {
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.oneEth_wei,              accounts[global.TaddrNdx + 0], 'TEST', );
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 1], CONST.nullFees, 0, [], [], { from: accounts[0] });
        ...
        assert(contractOwnerVcsTokQtyAfter == Number(contractOwnerVcsTokQtyBefore) + Number(expectedFeeCarbon), 'unexpected contract owner (fee receiver B) NATURE ST quantity after transfer');
    });

    it(`cross-entity transfer (different fee owners) - fees (percentage) - should round fees to zero for minimal transfers (ccy & carbon)`, async () => {
        ...
        let owner_balBefore = data.feeOwnerLedgerForA_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
        let owner_balAfter  =  data.feeOwnerLedgerForA_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
        assert(owner_balAfter == Number(owner_balBefore) + Number(expectedFeeCcy), 'unexpected contract owner (fee receiver A) USD balance after transfer');
        ...
        // test contract owner has received expected carbon CORSIA fee
        let contractOwnerCarbonTokQtyBefore = data.feeOwnerLedgerForA_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        let contractOwnerCarbonTokQtyAfter  =  data.feeOwnerLedgerForA_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        assert(contractOwnerCarbonTokQtyAfter == Number(contractOwnerCarbonTokQtyBefore), 'unexpected contract owner (fee receiver A) CORSIA ST quantity after transfer');

        contractOwnerCarbonTokQtyBefore = data.feeOwnerLedgerForB_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        contractOwnerCarbonTokQtyAfter  =  data.feeOwnerLedgerForB_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        assert(contractOwnerCarbonTokQtyAfter == Number(contractOwnerCarbonTokQtyBefore) + Number(expectedFeeCarbon), 'unexpected contract owner (fee receiver B) CORSIA ST quantity after transfer');
    });
    
    it(`cross-entity transfer (different fee owners) - fees (percentage) - should not allow non-owner to set global fee structure (ccy)`, async () => {
        try {
        ...
        try {
            const tx1 = await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.USD, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: 1, fee_min: 0, fee_max: 0 }, { from: accounts[10] });
        } catch (ex) { 
            assert(ex.reason == 'Restricted', `unexpected: ${ex.reason}`);
            return;
        }
    ...
    it(`cross-entity transfer (different fee owners) - fees (percentage) - should not allow non-owner to set global fee structure (carbon)`, async () => {
        try {
    ...
    it(`cross-entity transfer (different fee owners) - fees (percentage) - should not allow a transfer with insufficient ccy to cover fees`, async () => {
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD, CONST.millionCcy_cents,        accounts[global.TaddrNdx + 0], 'TEST', );
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T1, CONST.KT_CARBON, 1,  accounts[global.TaddrNdx + 1], CONST.nullFees, 0, [], [], { from: accounts[0] });
    ...
    it(`cross-entity transfer (different fee owners) - fees (percentage) - should not allow a transfer with insufficient carbon to cover fees`, async () => {
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD, CONST.millionCcy_cents,  accounts[global.TaddrNdx + 0], 'TEST', );
