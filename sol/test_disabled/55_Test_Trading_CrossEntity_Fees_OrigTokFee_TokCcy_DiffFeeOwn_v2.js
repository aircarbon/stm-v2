...
contract("DiamondProxy", accounts => {
    ...
    before(async function () {
        ...
        await stmStErc20Facet.createEntityBatch([
            {id: CONST.testId2, addr: CONST.testAddr10},
            {id: CONST.testId3, addr: CONST.testAddr11},
            {id: CONST.testId4, addr: CONST.testAddr12},
            {id: CONST.testId5, addr: CONST.testAddr13}
        ]);
    });
    ...
    beforeEach(async () => {
        ...
        await stmStErc20Facet.setAccountEntityBatch(
            [
                {id: CONST.testId1, addr: accounts[global.TaddrNdx + 0]},
                {id: CONST.testId2, addr: accounts[global.TaddrNdx + 1]},
                {id: CONST.testId3, addr: accounts[global.TaddrNdx + 2]},
                {id: CONST.testId4, addr: accounts[global.TaddrNdx + 3]},
                {id: CONST.testId5, addr: accounts[global.TaddrNdx + 4]},
            ]
        );
    ...
    it(`cross-entity transfer (different fee owners) - fees (orig token fee) - apply NATURE token 1 originator fee (+ ledger @ x4) [/ ETH global fee], on a 1.5 ST trade (tok fee on A / ccy fee on B)`, async () => {
        ...
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, A,              ledgerFeeTok);
        await stmStFeesFacet.setFee_TokType(2, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 1, fee_percBips: 0, fee_min: 0, fee_max: 0 }); // to test ledger override
        ...
        let owner_balVcsBefore = data.feeOwnerLedgerForA_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        let owner_balVcsAfter  =  data.feeOwnerLedgerForA_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(data.exchangeFee_tok_A).eq(Big(ledgerFeeTok.fee_fixed)), 'unexpected contract owner A token balance after transfer (1)');
        assert(Big(owner_balVcsAfter).eq(Big(owner_balVcsBefore).plus(Big(data.exchangeFee_tok_A))), 'unexpected contract owner A token balance after transfer (2)');

        owner_balVcsBefore = data.feeOwnerLedgerForB_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balVcsAfter  =  data.feeOwnerLedgerForB_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T2).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(data.exchangeFee_tok_B).eq(Big('0')), 'unexpected contract owner B token balance after transfer (1)');
        assert(Big(owner_balVcsAfter).eq(Big(owner_balVcsBefore)), 'unexpected contract owner B token balance after transfer (2)');
        
        let owner_balEthBefore = data.feeOwnerLedgerForA_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH)[0].balance;
        let owner_balEthAfter  =  data.feeOwnerLedgerForA_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH)[0].balance;
        assert(Big(owner_balEthAfter).eq(Big(owner_balEthBefore)), 'unexpected contract owner A currency balance after transfer');

        owner_balEthBefore = data.feeOwnerLedgerForB_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH)[0].balance;
        owner_balEthAfter  =  data.feeOwnerLedgerForB_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.ETH)[0].balance;
        assert(Big(owner_balEthAfter).eq(Big(owner_balEthBefore).plus(Big(data.exchangeFee_ccy_B))), 'unexpected contract owner B currency balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (orig token fee) - apply CORSIA token 1 originator fees (+ global @ x8) / SGD ledger fee, on a 2.5 ST trade (tok fee on B / ccy fee on A)`, async () => {
        ...
        await stmStFeesFacet.setFee_TokType(3, CONST.tokenType.TOK_T1, CONST.nullAddr, globalFeeTok);
        await stmStFeesFacet.setFee_TokType(3, CONST.tokenType.TOK_T1, B,              CONST.nullFees);
        ...
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.USD, A,                ledgerFeeCcy);
        await stmStFeesFacet.setFee_CcyType(2, CONST.ccyType.USD, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 200, fee_percBips: 0, fee_min: 0, fee_max: 0 } ); // to test ledger override
        ...
        // feeOwnerLedgerForA != feeOwnerLedgerForB
        let owner_balBefore = data.feeOwnerLedgerForA_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        let owner_balAfter  =  data.feeOwnerLedgerForA_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfter).eq(Big(owner_balBefore)), 'unexpected fee receiver A token balance after transfer');

        owner_balBefore = data.feeOwnerLedgerForB_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balAfter  =  data.feeOwnerLedgerForB_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfter).eq(Big(owner_balBefore).plus(Big(data.exchangeFee_tok_B))), 'unexpected fee receiver B token balance after transfer');
        
        let owner_balCcyBefore = data.feeOwnerLedgerForA_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD)[0].balance;
        let owner_balCcyAfter  =  data.feeOwnerLedgerForA_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD)[0].balance;
        assert(Big(owner_balCcyAfter).eq(Big(ledgerFeeCcy.fee_fixed)), 'unexpected contract owner A currency balance after transfer (1)');
        assert(Big(owner_balCcyAfter).eq(Big(owner_balCcyBefore).plus(Big(data.exchangeFee_ccy_A))), 'unexpected A contract owner currency balance after transfer (2)');

        owner_balCcyBefore = data.feeOwnerLedgerForB_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD)[0].balance;
        owner_balCcyAfter  =  data.feeOwnerLedgerForB_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD)[0].balance;
        assert(Big(owner_balCcyAfter).eq(Big(owner_balCcyBefore)), 'unexpected contract owner B currency balance after transfer (2)');
    ...
    it(`cross-entity transfer (different fee owners) - fees (orig token fees) - apply NATURE token multiple [3] originator fees (+ ledger @ x4), on a 3.5 ST trade (fee on A)`, async () => {
        ...
        for (var i = 0 ; i < M_multi.length ; i++) {
            ...
            await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,                      CONST.oneEth_wei,        A, 'TEST');
            await stmStFeesFacet.setFee_TokType(i + 1, CONST.tokenType.TOK_T2,       CONST.nullAddr,          CONST.nullFees);
            await stmStFeesFacet.setFee_CcyType(i + 1, CONST.ccyType.ETH,            CONST.nullAddr,          CONST.nullFees);
            const data_MA = await transferHelper.transferLedger({ stmStLedgerFacet, stmStFeesFacet, stmStTransferableFacet, stmStErc20Facet, accounts, 
                ledger_A: M,                                   ledger_B: A,
                   qty_A: new BN(MA_qty),                   tokTypeId_A: CONST.tokenType.TOK_T2,
                   qty_B: 0,                                tokTypeId_B: 0,
        ...
        await stmStFeesFacet.setFee_TokType(4, CONST.tokenType.TOK_T2, CONST.nullAddr, CONST.nullFees);
        ...
        await stmStFeesFacet.setFee_TokType(4, CONST.tokenType.TOK_T2, A, ledgerFees);
        ...
        assert(Big(owner_balAfter).eq(Big(owner_balBefore).plus(Big(data.exchangeFee_tok_A))), 'unexpected contract owner A token balance after transfer');
        ...
        assert(Big(owner_balAfter).eq(Big(owner_balBefore)), 'unexpected contract owner B token balance after transfer');
    ...
    it(`cross-entity transfer (different fee owners) - fees (orig token fees) - apply CORSIA token multiple [3] originator fees (+ global @ x10), on a 3.5 ST trade (fee on B)`, async () => {
        ...
        for (var i = 0 ; i < M_multi.length ; i++) {
            ...
            const MA_qty = CONST.KT_CARBON;
            await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,                   CONST.oneEth_wei,        B, 'TEST');
            await stmStFeesFacet.setFee_TokType(i + 1, CONST.tokenType.TOK_T1,    CONST.nullAddr,          CONST.nullFees);
            await stmStFeesFacet.setFee_CcyType(i + 1, CONST.ccyType.ETH,         CONST.nullAddr,          CONST.nullFees);
            const data_MA = await transferHelper.transferLedger({ stmStLedgerFacet, stmStFeesFacet, stmStTransferableFacet, stmStErc20Facet, accounts, 
                ledger_A: M,                                   ledger_B: B,
                   qty_A: new BN(MA_qty),                   tokTypeId_A: CONST.tokenType.TOK_T1,
        ...
        await stmStFeesFacet.setFee_TokType(5, CONST.tokenType.TOK_T1, CONST.nullAddr, CONST.nullFees);
        ...
        await stmStFeesFacet.setFee_TokType(5, CONST.tokenType.TOK_T1, B, CONST.nullFees);
        await stmStFeesFacet.setFee_TokType(5, CONST.tokenType.TOK_T1, CONST.nullAddr, globalFee);
        ...
        let owner_balBefore = data.feeOwnerLedgerForA_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        let owner_balAfter  =  data.feeOwnerLedgerForA_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfter).eq(Big(owner_balBefore)), 'unexpected contract owner A token balance after transfer');

        owner_balBefore = data.feeOwnerLedgerForB_before.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balAfter  =  data.feeOwnerLedgerForB_after.tokens.filter(p => p.tokTypeId == CONST.tokenType.TOK_T1).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        assert(Big(owner_balAfter).eq(Big(owner_balBefore).plus(Big(data.exchangeFee_tok_B))), 'unexpected contract owner B token balance after transfer');
