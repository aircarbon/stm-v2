...
contract("DiamondProxy", accounts => {
    ...
    it(`cross-entity transfer (different fee owners) - fees (ccy per million received, mirrored) - apply mirrored USD ccy fee 3 USD/1m tokens received on trades (0.1KT, 1KT, 11KT, 15KT) (global fee on A)`, async () => {
        ...
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 1, CONST.ccyType.USD, CONST.nullAddr)).ccy_perMillion == ccy_perMillion, 'unexpected fee per Million received after setting ccy fee structure (entity 1)');
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 2, CONST.ccyType.USD, CONST.nullAddr)).ccy_perMillion == ccy_perMillion, 'unexpected fee per Million received after setting ccy fee structure (entity 2)');
    ...
    it(`cross-entity transfer (different fee owners) - fees (ccy per million received, mirrored) - apply mirrored USD ccy fee 3 USD/1m tokens received on trades (0.1KT, 1KT, 11KT, 15KT) (global fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0], B = accounts[global.TaddrNdx + 1];
    ...
    it(`cross-entity transfer (different fee owners) - fees (ccy per million received, mirrored) - apply asymmetrical mirrored ledger override USD ccy fee 6 USD/1m tokens received, capped USD 60, on trades (0.1KT, 1KT, 11KT, 15KT) (ledger fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0], B = accounts[global.TaddrNdx + 1];
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD,                      CONST.millionCcy_cents,  A, 'TEST');
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.MT_CARBON,  1,     B, CONST.nullFees, 0, [], [], { from: accounts[0] });
        ...
        for (var i = 0 ; i < transferAmountsTok.length ; i++) {
            ...
            let owner_balBefore = data.feeOwnerLedgerForA_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
            let owner_balAfter  =  data.feeOwnerLedgerForA_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
            assert(owner_balAfter == Number(owner_balBefore) + Number(expectedFeeCcy_A), 'unexpected contract owner (fee receiver A) ccy balance after transfer');
            
            owner_balBefore = data.feeOwnerLedgerForB_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
            owner_balAfter  =  data.feeOwnerLedgerForB_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
            assert(owner_balAfter == Number(owner_balBefore) + Number(expectedFeeCcy_B), 'unexpected contract owner (fee receiver B) ccy balance after transfer');
        }
    });

    it(`cross-entity transfer (different fee owners) - fees (ccy per million received, mirrored) - apply asymmetrical mirrored ledger override USD ccy fee 6 USD/1m tokens received, capped USD 60, on trades (0.1KT, 1KT, 11KT, 15KT) (ledger fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0], B = accounts[global.TaddrNdx + 1];
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.MT_CARBON,  1,     A, CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD,                      CONST.millionCcy_cents,  B, 'TEST');
    ...
    it(`cross-entity transfer (different fee owners) - fees (ccy per million received, mirrored) - insufficent balance on mirror (B) - mirrored USD ccy fee 3 USD/1m tokens received on trades (1KT, 1.5KT, 2.0KT) (global fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0], B = accounts[global.TaddrNdx + 1];
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD,                      CONST.millionCcy_cents,  A, 'TEST');
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.MT_CARBON,  1,     B, CONST.nullFees, 0, [], [], { from: accounts[0] });
    ...
    it(`cross-entity transfer (different fee owners) - fees (ccy per million received, mirrored) - insufficent balance on mirror (A) - mirrored USD ccy fee 3 USD/1m tokens received on trades (1KT, 1.5KT, 2.0KT) (global fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0], B = accounts[global.TaddrNdx + 1];
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.MT_CARBON,  1,     A, CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD,                      CONST.millionCcy_cents,  B, 'TEST');
