...
contract("DiamondProxy", accounts => {
    ...
    // ST MULTI FEES
    it(`cross-entity transfer (same fee owner) - fees (multi) - apply NATURE token fee 100 BP + 1 TONS fixed on a small trade (fee on A)`, async () => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi) - apply NATURE token fee 1000 BP + 1000 TONS fixed on a large (0.5 GT) trade (fee on B)`, async () => {
    ...
    // CCY MULTI FEES
    it(`cross-entity transfer (same fee owner) - fees (multi) - apply ETH ccy fee 100 BP + 0.01 ETH fixed on a small trade (fee on A)`, async () => {
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.oneEth_wei,              accounts[global.TaddrNdx + 0], 'TEST', );
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi) - apply ETH ccy fee 1000 BP + 1000 ETH fixed on a large (500k ETH) trade (fee on B)`, async () => {
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,   CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 0], CONST.nullFees, 0, [], [], { from: accounts[0] });
    ...
    // CCY + ST MULTI FEES
    it(`cross-entity transfer (same fee owner) - fees (multi) - apply ETH ccy fee 1000 BP + 1000 ETH fixed, NATURE fee 1000 BP + 1000 TONS on a large (500k ETH / 0.5GT) trade (fees on both sides)`, async () => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi) - should not allow a transfer with insufficient ccy (fixed + percentage) to cover fees (fee on A)`, async () => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi) - should not allow a transfer with insufficient ccy (fixed + percentage) to cover fees (fee on B)`, async () => {
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi) - should not allow a transfer with insufficient carbon to cover fees (fee on A)`, async () => {
        // 102,999,999 tons
    ...
    it(`cross-entity transfer (same fee owner) - fees (multi) - should not allow a transfer with insufficient carbon to cover fees (fee on B)`, async () => {
        // 102,999,999 tons
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,       CONST.oneEth_wei,              accounts[global.TaddrNdx + 0], 'TEST', );
