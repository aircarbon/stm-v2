...
contract("DiamondProxy", accounts => {
    ...
    it(`fees (fixed) - apply USD custom ccy fee on a max. trade (no global fees) (fee on B)`, async () => {
        ...
        const data = await transferHelper.transferLedgerCustomFee({ stmStLedgerFacet, stmStFeesFacet, stmStTransferableFacet, stmStErc20Facet, accounts, 
                ledger_A: accounts[global.TaddrNdx + 0],                                      ledger_B: accounts[global.TaddrNdx + 1],
                   ...
                   qty_B: 0,                                                               tokTypeId_B: 0,
            ccy_amount_A: 0,                                                               ccyTypeId_A: 0,
            ccy_amount_B: new BN(CONST.millionCcy_cents).sub(new BN(customFee_A)),         ccyTypeId_B: CONST.ccyType.USD,
               applyFees: true,
               feeA: customFee_A,                                                                 feeB: 0
        });
        ...
        const owner_balBefore = data.feeOwnerLedgerForA_before.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
        const owner_balAfter  =  data.feeOwnerLedgerForA_after.ccys.filter(p => p.ccyTypeId == CONST.ccyType.USD).map(p => p.balance).reduce((a,b) => Number(a) + Number(b), 0);
        assert(owner_balAfter == Number(owner_balBefore) + Number(customFee_A), 'unexpected contract owner (fee receiver) USD balance after transfer');
    });

    it(`fees (fixed) - apply USD custom ccy fee on a max. trade (with global fee) (fee on B)`, async () => {
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 0], CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD,        CONST.millionCcy_cents,  accounts[global.TaddrNdx + 1], 'TEST', );
    ...
    it(`fees (fixed) - apply USD custom ccy fee on a max. trade (with specific addr fee) (fee on B)`, async () => {
        const B = accounts[global.TaddrNdx + 1];
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 0], CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD,        CONST.millionCcy_cents,  accounts[global.TaddrNdx + 1], 'TEST', );
    ...
    it(`fees (fixed) - apply USD custom ccy zero fee on a max. trade (with specific addr fee) (fee on B)`, async () => {
        const B = accounts[global.TaddrNdx + 1];
        await stmStMintableFacet.mintSecTokenBatch(CONST.tokenType.TOK_T2,    CONST.KT_CARBON, 1,      accounts[global.TaddrNdx + 0], CONST.nullFees, 0, [], [], { from: accounts[0] });
        await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.USD,        CONST.millionCcy_cents,  accounts[global.TaddrNdx + 1], 'TEST', );
    ...
    it(`custom fees (fixed) - apply USD custom ccy fee on a max. trade (with originator fees) (fees on A and B)`, async () => {
        ...
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 0, CONST.ccyType.USD, B)).fee_fixed == 0, 'unexpected USD fixed cent fee before setting USD fee structure for B');
        let setFeeTx = await stmStFeesFacet.setFee_CcyType(0, CONST.ccyType.USD, B, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: FeeFixedA_cent, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
        truffleAssert.eventEmitted(setFeeTx, 'SetFeeCcyFix', ev => ev.ccyTypeId == CONST.ccyType.USD && ev.fee_ccy_Fixed == FeeFixedA_cent && ev.ledgerOwner == B);
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 0, CONST.ccyType.USD, B)).fee_fixed == FeeFixedA_cent, 'unexpected USD fixed cent fee after setting USD fee structure for B');

        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 0, CONST.ccyType.USD, A)).fee_fixed == 0, 'unexpected USD fixed cent fee before setting USD fee structure for A');
        setFeeTx = await stmStFeesFacet.setFee_CcyType(0, CONST.ccyType.USD, A, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: FeeFixedB_cent, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
        truffleAssert.eventEmitted(setFeeTx, 'SetFeeCcyFix', ev => ev.ccyTypeId == CONST.ccyType.USD && ev.fee_ccy_Fixed == FeeFixedB_cent && ev.ledgerOwner == A);
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 0, CONST.ccyType.USD, A)).fee_fixed == FeeFixedB_cent, 'unexpected USD fixed cent fee after setting USD fee structure for A');
