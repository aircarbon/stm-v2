...
contract("DiamondProxy", accounts => {
    ...
    it(`fees (percentage) - apply CORSIA token fee 1 BP (min) on a trade 1000 tons (min lot size) (fee on B)`, async () => {
        ...
        const setFeeTx = await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T1, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: feeBips, fee_min: 0, fee_max: 0 } );
        ...
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.TOK, 1, CONST.tokenType.TOK_T1, CONST.nullAddr)).fee_percBips == feeBips, 'unexpected CORSIA percentage fee after setting CORSIA fee structure');
    ...
    it(`fees (percentage) - apply USD ccy fee 1 BP on a trade (fee on B)`, async () => {
        ...
        const setFeeTx = await stmStFeesFacet.setFee_CcyType(1, CONST.ccyType.USD, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: usdFeePercBips, fee_min: 0, fee_max: 0 } );
        ...
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 1, CONST.ccyType.USD, CONST.nullAddr)).fee_percBips == usdFeePercBips, 'unexpected USD percentage fee after setting USD fee structure');
    ...
    it(`fees (percentage) - apply ccy fee 50 BP on a trade on a newly added ccy`, async () => {
        ...
        const setFeeTx = await stmStFeesFacet.setFee_CcyType(1, newCcyTypeId, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: feeBips, fee_min: 0, fee_max: 0 } );
        ...
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.CCY, 1, newCcyTypeId, CONST.nullAddr)).fee_percBips == feeBips, 'unexpected new ccy percentage fee after setting fee structure');
    ...
    it(`fees (percentage) - apply ETH ccy & NATURE ST fee on a 0.5 ST trade (fees on both sides)`, async () => {
        ...
        const setCarbonFeeTx = await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T2, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: carbonFeeBps, fee_min: 0, fee_max: 0 } );
        ...
        assert((await stmStFeesFacet.getFee(CONST.getFeeType.TOK, 1, CONST.tokenType.TOK_T2, CONST.nullAddr)).fee_percBips == carbonFeeBps, 'unexpected NATURE basis points fee after setting NATURE fee structure');
    ...
    it(`fees (percentage) - should round fees to zero for minimal transfers (ccy & carbon)`, async () => {
        ...
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T1, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: carbonFeeBps, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - should not allow a transfer with insufficient ccy to cover fees`, async () => {
        ...
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T1, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - should not allow a transfer with insufficient carbon to cover fees`, async () => {
        ...
        await stmStFeesFacet.setFee_TokType(1, CONST.tokenType.TOK_T1, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perMillion: 0, fee_fixed: 0, fee_percBips: 1, fee_min: 0, fee_max: 0 } );
