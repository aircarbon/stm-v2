...
contract("DiamondProxy", accounts => {
    ...
    beforeEach(async () => {
        ...
        await stmStErc20Facet.setAccountEntityBatch(
            [
                {id: CONST.testId1, addr: accounts[global.TaddrNdx + 1]},
                {id: CONST.testId2, addr: accounts[global.TaddrNdx + 2]},
                {id: CONST.testId3, addr: accounts[global.TaddrNdx + 3]},
                {id: CONST.testId4, addr: accounts[global.TaddrNdx + 4]},
                {id: CONST.testId5, addr: accounts[global.TaddrNdx + 5]},
            ]
        );
    ...
    it(`cross-entity transfer (different fee owners) - fees (orig/orig) - apply NATURE token M originator fees (+ ledger fee) / CORSIA token M originator fees (+ global fee), on a 1.5 ST trade (tok fee on A / tok fee on B)`, async () => {
        ...
        var { qty: qty_A, M_multi: M_multi_A } = await MintAndTransfer({ tokenType: tokType_A, originatorCount: 2, batchesPerOriginator: 1, transferTo: A, transferToEntId: 1 });
        var { qty: qty_B, M_multi: M_multi_B } = await MintAndTransfer({ tokenType: tokType_B, originatorCount: 1, batchesPerOriginator: 2, transferTo: B, transferToEntId: 2 });
        ...
        await stmStFeesFacet.setFee_TokType(1, tokType_A, A,              ledgerFee_A);
        await stmStFeesFacet.setFee_TokType(1, tokType_A, CONST.nullAddr, CONST.nullFees);
        ...
        await stmStFeesFacet.setFee_TokType(2, tokType_B, CONST.nullAddr, globalFee_B);
        await stmStFeesFacet.setFee_TokType(2, tokType_B, B,              CONST.nullFees);

        // TEST - transfer
        qty_A = qty_A.minus(Big(500)); // take off some for fees
        qty_B = qty_B.minus(Big(500));

        for (var i = 0 ; i < M_multi_A.length ; i++) M_multi_A[i].ledgerBefore = await stmStLedgerFacet.getLedgerEntry(M_multi_A[i].account);
        for (var i = 0 ; i < M_multi_B.length ; i++) M_multi_B[i].ledgerBefore = await stmStLedgerFacet.getLedgerEntry(M_multi_B[i].account);
        ...
        for (var i = 0 ; i < M_multi_A.length ; i++) M_multi_A[i].ledgerAfter = await stmStLedgerFacet.getLedgerEntry(M_multi_A[i].account);
        for (var i = 0 ; i < M_multi_B.length ; i++) M_multi_B[i].ledgerAfter = await stmStLedgerFacet.getLedgerEntry(M_multi_B[i].account);
        ...
        let owner_balBefore = data.feeOwnerLedgerForA_before.tokens.map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        let owner_balAfter  =  data.feeOwnerLedgerForA_after.tokens.map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        ...
        owner_balBefore = data.feeOwnerLedgerForB_before.tokens.map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        owner_balAfter  =  data.feeOwnerLedgerForB_after.tokens.map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
    ...
    async function MintAndTransfer(a) {
        ...
        for (var i = 0 ; i < M_multi.length ; i++) {
            ...
            for (var j = 0 ; j < batchesPerOriginator ; j++) {
                ...
                await stmStMintableFacet.mintSecTokenBatch(tokenType, qty, 1, M, origFee, 0, [], [], { from: accounts[0] });
                totalTokQty += qty;
            }
        }
        ...
        for (var i = 0 ; i < M_multi.length ; i++) {
            const M = M_multi[i].account;
            const le = await stmStLedgerFacet.getLedgerEntry(M);
            const M_qty = le.tokens.filter(p => p.tokTypeId == tokenType).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));

            await stmCcyCollateralizableFacet.fundOrWithdraw(CONST.fundWithdrawType.FUND, CONST.ccyType.ETH,                   CONST.oneEth_wei,        transferTo, 'TEST');
            await stmStFeesFacet.setFee_TokType(i + 3, tokenType,                 CONST.nullAddr,          CONST.nullFees); // no exchange fees
            await stmStFeesFacet.setFee_CcyType(a.transferToEntId, CONST.ccyType.ETH,         CONST.nullAddr,          CONST.nullFees);
        ...
        const le = await stmStLedgerFacet.getLedgerEntry(transferTo);
